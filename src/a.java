...
public class a extends Applet implements Runnable {
  ...
  public void run() {
    ...
    while(true) {
      ...
      if (level < 84) {
        ...
        g.setColor(v);
        g.drawString("demon atack", 3, 203);
        g.drawString("score:"+record,120, 203);

        if(cheat){
          g.setColor(Color.WHITE);
          g.setFont(new Font("Monotype Corsiva",Font.ITALIC,12));
          g.drawString("trapaça",75,203);
        }
  ...
  public void processKeyEvent(KeyEvent keyEvent) {
    ...
    if (k > 0) {
      ...
      a[(k == VK_LEFT || k == VK_RIGHT || k == VK_P || k == VK_T || k == VK_u) ? k : VK_SHOOT]
          = keyEvent.getID() != 402;
    }
    if (k==VK_T) {
      cheat=true;
